  ...
  <div class="mt-3">
    <div class="row no-gutters d-flex">
      ...
      <button
        ...
        v-on:click="this.openModal(null)"
      >
        Novo vendedor
      </button>
    </div>
          ...
          <tr v-for="employee in employees" :key="employee.id">
            ...
            <td>
              <button
                ...
                v-on:click="this.delete(employee.id)"
              >
                Excluir</button
              >&nbsp;
              <button
                ...
                v-on:click="this.openModal(employee)"
              >
                Editar
              </button>
            </td>
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="employeeModalLabel">
            {{ !this.newEmployee ? "Editar vendedor" : "Cadastrar vendedor" }}
          </h5>
          <button
            type="button"
            class="close"
        ...
        <form id="formEmployee" action="#">
          ...
          <div class="modal-footer">
            ...
            <button
              type="button"
              class="btn btn-primary"
              v-on:click="this.saveEmployee()"
            >
              {{ !this.newEmployee ? "Editar" : "Cadastrar" }}
            </button>
          </div>
        </form>
      </div>
  ...
  methods: {
    ...
    saveEmployee() {
      ...
      if (this.editForm.id != null) {
        axios
          .post("/api/employee/update/" + this.editForm.id, this.editForm)
          .then((res) => {
        ...
        //Cadastro
        axios
          .post("/api/employee/create", this.editForm)
          .then((res) => {
            if (res.data.status == "success") {
